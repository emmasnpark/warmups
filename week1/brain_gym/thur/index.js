...
function ranStr() {
  var thi = thisArray.length;
  var tha = thatArray.length;
  var newArr = [];
  for(i=0; i<=4; i++) {
    var ranThisArray = Math.floor((Math.random() * thi) + 1);
    ranThisArray = ranThisArray - 1;
    var ranThatArray = Math.floor((Math.random() * tha) + 1);
    ranThatArray = ranThatArray -1;
    var strCom = thisArray[ranThisArray] + " for " + thatArray[ranThatArray];
    newArr.push(strCom);
    var test = (i+1)+" "+newArr[i];
    var n = i+1;
    document.getElementById(n).innerHTML = test;
  }
